...
static void interpretMarkerOptions(id json, id<FLTAMapMarkerOptionsSink> sink,
...
- (instancetype)initWithFrame:(CGRect)frame
    ...
    if ([super init]) {
        ...
        _mapView = [MAMapView mapWithFrame:frame camera:camera];
...
- (void)setMapType:(MAMapViewType)mapType {
...
- (BOOL)mapView:(MAMapView*)mapView didTapMarker:(GMSMarker*)marker {
    NSString* markerId = marker.userData[0];
    [_delegate onMarkerTappedOnMap:_mapId marker:markerId];
    return [marker.userData[1] boolValue];
}

- (void)mapView:(MAMapView*)mapView didTapInfoWindow:(GMSMarker*)marker {
    NSString* markerId = marker.userData[0];
...
static MAMapViewType toMapViewType(id json) {
    int value = toInt(json);
    return (MAMapViewType)(value == 0 ? 5 : value);
}
